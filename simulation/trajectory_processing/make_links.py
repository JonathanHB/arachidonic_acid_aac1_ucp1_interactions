...
linkpath = "/media/X01Raid01/Data_Backup/home/jborowsky/long-sims/long-aac1-ucp1-processing/centered_trajectories_links"
                ...
                if os.path.exists(fpath):
                    ...
                    if server == "wynton":
                        ...
                        for seg in ["001"]+[str(i) for i in range(100, 1000, 100)]:
                            ...
                            if len(xtcfile) == 1:
                                trjpath = f"{fpath}/{xtcfile[0]}"
                                
                                segstr = str(x).zfill(2)
                                os.system(f"ln -s {trjpath} {linkpath}/{server}/{protein}/run0{run}/seg{segstr}.xtc")
                                x+=1
                                #observable(calc_obs, server, protein, run, seg, f"{fpath}/{xtcfile[0]}", toppath)
                    ...
                    if server == "degrabo":
                        ...
                        for seg in range(20):
                            ...
                            if os.path.exists(trjpath):
                                ...
                                os.system(f"ln -s {trjpath} {linkpath}/{server}/{protein}/run0{run}/seg{segstr}.xtc")
                                x+=1
                                #observable(calc_obs, server, protein, run, seg, trjpath, toppath)
                                #total_frames += frames
...
#    print(data_lines)
#    out_path = f"/media/X01Raid01/Data_Backup/home/jborowsky/long-sims/long-aac1-ucp1-processing/frame_counts.txt"
